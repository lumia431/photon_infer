...
Result<ModelLoader::LoadedModelTyped<WeightType>> ModelLoader::load_typed(
    const std::filesystem::path& model_path) {
  ...
  return Ok(LoadedModelTyped<WeightType>{
...
template Result<ModelLoader::LoadedModelTyped<f32>> ModelLoader::load_typed<f32>(
    const std::filesystem::path&);

template Result<ModelLoader::LoadedModelTyped<i8>> ModelLoader::load_typed<i8>(
    const std::filesystem::path&);
